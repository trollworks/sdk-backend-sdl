...
namespace tw::sdl::rendering {
  ...
  void blend_node::on_allocate(SDL_Renderer* renderer, Uint32 format, SDL_Point size) {
  ...
  void blend_node::on_update(float delta_time) {
    for (const auto& node : m_nodes) {
      node->update(delta_time);
    }
  }

  void blend_node::on_apply(SDL_Renderer* renderer) {
    for (const auto& node : m_nodes) {
      node->apply(renderer);
      SDL_SetTextureBlendMode(node->target(), SDL_BLENDMODE_BLEND);
